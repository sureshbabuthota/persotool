...
namespace SmartCardNotifications
{
    /// <summary>
	...
	public class CardNative : CardBase
	{
        ...
        public override byte[] Transmit(byte[] ApduBuffer, int bufferLen)
        {
            uint RecvLength = 1024;
            byte[] ApduResponse = new byte[RecvLength];
            SCard_IO_Request ioRequest = new SCard_IO_Request();
            ioRequest.m_dwProtocol = m_nProtocol;
            ioRequest.m_cbPciLength = 8;

            m_nLastError = SCardTransmit(m_hCard, ref ioRequest, ApduBuffer, (uint)bufferLen, IntPtr.Zero, ApduResponse, out RecvLength);
            if (m_nLastError != 0)
            {
                string msg = "SCardTransmit error: " + m_nLastError;
                throw new Exception(msg);
        ...
        public override string GetATR()
        {
            string atrStr = "";
            byte[] atrbytes = GetAttribute(SCARD_ATTR_VALUE.ATR_STRING);

            if (atrbytes!=null)
            {
                for (int i = 0; i < atrbytes.Length; i++)
                {
                    string temp = atrbytes[i].ToString("X");
        ...
        protected override void RunCardDetection(object Reader)
        {
            ...
            if (SCardEstablishContext((uint) SCOPE.User, IntPtr.Zero, IntPtr.Zero, phContext) == 0)
            {
                ...
                do
                {
                    if (SCardGetStatusChange(hContext, WAIT_TIME, readerState, nbReaders) == 0)
                    {
                        ...
                        else if (readerState[0].m_cbAtr>5)
                        {
                            CardInserted((string)Reader);
                        }
